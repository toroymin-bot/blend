...
- "API 키 왜 등록해야 해?" / "꼭 키가 필요해?" → Blend는 BYOK(키 직접 등록) 모델. 키 없이 무료 trial(Gemini 2.5 Flash 50회/일)만 가능. 키 등록하면 1) 본인 OpenAI/Anthropic/Google 계정 사용량으로 직접 청구 → 평균 월 $5(구독 $60+ 대비 95% 절감) 2) 멀티 AI 자동 라우팅(코딩=Claude, 긴 문서=Gemini 등) 활용 3) 한도/모델 무제한.
...
- "Why do I need to register API keys?" / "Are keys required?" → Blend is BYOK. Without keys, only the free Gemini 2.5 Flash trial (50/day) works. Registering keys lets you 1) get billed directly via your own OpenAI/Anthropic/Google accounts → ~$5/mo average (95% off vs $60+ subscriptions) 2) use multi-AI auto-routing (coding=Claude, long docs=Gemini, etc.) 3) no Blend-side limits.
...
그 다음 섹션:
1. 컨셉 — 어떤 서비스이고 어떤 문제를 해결하는지. 핵심: Blend는 Claude + ChatGPT + Gemini를 한 화면에서 사용하는 통합 AI. 멤버십(월 $9 또는 6개월 $39 = 28% 할인) + API 사용료는 원가 그대로(마진 0%) — 그래서 매달 $60 구독료 대신 평균 $5로 모든 AI 가능.
...
4. 비용 절감 효과 — 멤버십 + 원가 API로 평균 월 $5. 일반 ChatGPT Plus + Claude Pro + Gemini Advanced 동시 구독 ($60+) 대비 약 90% 절감. 안 쓰면 $0.
...
Then sections:
1. Concept — what it is and what problem it solves. Core: Blend unifies Claude + ChatGPT + Gemini in one screen. Membership ($9/mo or $39/6mo = 28% off) + API usage at cost (0% markup) — so on average ~$5/month total instead of $60+ for separate subscriptions.
...
4. Cost savings — Membership + at-cost API averages ~$5/month total. Vs subscribing to ChatGPT Plus + Claude Pro + Gemini Advanced ($60+ combined) = ~90% savings. $0 when idle.
