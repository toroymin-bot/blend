...
function friendlyDataSourceError(raw: string | undefined, lang: 'ko' | 'en'): { what: string; how: string; openSettings?: boolean } {
  ...
  if (/maximum input length|input.*too long|context length|8192 tokens|token.*limit/.test(e)) {
    return ko
      ? { what: '파일 내용이 너무 길어요.', how: '더 짧은 문서로 나눠 업로드해주세요.' }
      : { what: 'File content too long.', how: 'Split into shorter pieces and re-upload.' };
  }
  if (/out of memory|allocation failed|maximum call stack|heap.*exceeded/.test(e)) {
    return ko
      ? { what: '메모리 부족으로 실패.', how: '데스크톱에서 시도하거나 더 작은 파일로.' }
      : { what: 'Out of memory.', how: 'Try on desktop or use smaller files.' };
  }
  if (/10분 초과|exceeded 10 min|file processing exceeded|file timeout/.test(e)) {
    return ko
      ? { what: '한 파일 처리 10분 초과.', how: '더 작은 파일로 나눠주세요 (다른 파일은 정상).' }
      : { what: 'File processing exceeded 10 min.', how: 'Split into smaller files (others synced fine).' };
  }
